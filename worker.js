...
const SITE_RUNTIME_VERSION = 'v2026.04.23.22';
...
export default {
  async fetch(request, env) {
    ...
    // Normalize: strip trailing slash except root — track whether we had one
    ...
    // 1. Exact-match redirects (single-hop — if slash was present, this also consolidates it)
    ...
    if (path === '/wv') {
      return new Response(JSON.stringify({worker:'v103',ru:true,gone410:true,leadFallback:true,trailingSlash301:true,ts:Date.now()}), {
    ...
    // 4. CRITICAL SEO FIX — consolidate trailing-slash duplicates with 301 (not 307).
    //    Previously Cloudflare Pages auto-served /foo/ with a 307 (temporary) redirect,
    //    which Google treats as non-canonical and indexes both versions. This caused
    //    the 31 slash-split pairs seen in GSC and contributed to de-indexing (245→104).
    //    Issuing an explicit 301 BEFORE the ASSETS fallback consolidates link equity
    //    and gives Google a clear canonical signal.
    if (hadTrailingSlash) {
      return Response.redirect(new URL(path + url.search + url.hash, url.origin).href, 301);
    }
